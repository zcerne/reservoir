...
class Reservoir:
    ...
    def _sted_susceptibilities(self):
        """Cached STED MultilevelAtom list (built ONCE, shared between the reservoir
        material function AND get_extra_materials — MEEP identifies a susceptibility
        by object, so the same atom must appear in both)."""
        if getattr(self, "_sted_susc_cache", None) is None:
            self._sted_susc_cache = self._build_sted_susceptibilities()
        return self._sted_susc_cache

    def get_extra_materials(self):
        """Media that appear ONLY inside a material function must also be handed to
        `mp.Simulation(extra_materials=...)` so MEEP allocates the polarization fields
        for their susceptibilities — otherwise the MultilevelAtom is silently ignored.
        (This is the missing piece the wgm_lasing reference has.) Empty without STED."""
        import meep as mp
    ...
    def get_geometry_blocks(self, mp_module=None):
        ...
        _sted_susc = self._sted_susceptibilities()   # [] unless reservoir.sted set; SHARED
