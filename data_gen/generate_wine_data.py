"""Generate (input voltages, output intensity) pairs for the wine dataset
by running the voltage_reservoir + FDTD pipeline on each sample.

Wine variant of generate_iris_data.py — 13 features → 13 voltages, y-symmetric
(voltages_y_min[k] = voltages_y_max[k]), top + bottom electrodes mirror.

Default design: data/2_2D_electrodes (26×8 µm reservoir, 13 electrodes per
face at 2 µm pitch). Default output: data/NN_data/4_electrodes_wine.
"""
...
DESIGN_FOLDER = HERE / "data" / "2_2D_electrodes"
OUT_FOLDER = HERE / "data" / "NN_data" / "4_electrodes_wine"


def load_wine() -> tuple[np.ndarray, np.ndarray, list[str]]:
    from sklearn import datasets
    wine = datasets.load_wine()
    return (np.asarray(wine.data, dtype=np.float64),
            np.asarray(wine.target, dtype=np.int64),
            [str(s) for s in wine.target_names])
...
def main():
    ...
    for i in range(X.shape[0]):
        ...
        if n_y is None:
            ...
            raise RuntimeError(f"sample {i} sensor length {I_out.size} vs {n_y}")
        I_outs.append(I_out)
    ...
    out_path = out_folder / "wine_dataset.npz"
    np.savez(out_path,
