...
def run_one(level, pump_amp):
    ...
    # single-element list -> return_quadrosource/_strip_sources makes ONE strip
    # spanning the FULL size.y (10um) -- equivalent to one continuous line source,
    # not the 2-strip split used elsewhere.
    sim.amp_override = {"source_1": [level], "source_pump": [pump_amp]}
    sim.args = {}
    sim.run()
...
for lv in levels:
    ...
    print(f"[single-line-gain] level={lv:g}  out_pump_on={out_on:.6g}  out_pump_off={out_off:.6g}  gain={out_on/out_off:.6g}", flush=True)
