...
for RES in (40, 60, 80):
    p = build_json(4)
    # override resolution in the design JSON
    jf = p + "/simulation_data.json"
    cfg = json.load(open(jf)); cfg["resolution"]=RES
    json.dump(cfg, open(jf,"w"))
    ...
    print("RES=%d complex-corr=%.4f max-ratio=%.4f rel-L2=%.3f (MEEP max %.4f gpu %.4f)" % (
      RES, np.abs(np.vdot(b,a))/(np.linalg.norm(b)*np.linalg.norm(a)),
      np.abs(b).max()/np.abs(a).max(), np.linalg.norm(b-a)/np.linalg.norm(a), np.abs(m).max(), np.abs(g).max()))
print("DONE")
