...
BASE = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                    "data", "twoelectrode", "focus12_anchored")
GAP, SPAN, NCTRL, VMAX = 12.0, 24.0, 12, 3.5
TARGET_SIGMA = 1.5           # Gaussian target width (um) at y=0
...
def cost(coeffs, grad=None):
    ...
    # Gaussian-weighted focus fraction: Σ I·G / Σ I with G = exp(-y²/2σ²)
    G = np.exp(-y ** 2 / (2.0 * TARGET_SIGMA ** 2))
    frac = (I * G).sum() / max(I.sum(), 1e-300)
    c = 1.0 - frac
...
def cost_sym(half, grad=None):
    return cost(_mirror(half))
...
def main():
    ...
    opt.set_upper_bounds([VMAX] * n)
    opt.set_min_objective(cost_sym)
    ...
    if os.path.exists(hist):
        ...
        x0 = np.asarray(h["coeffs"][k][:n], dtype=np.float64)
        print(f"[opt] resuming from best of {len(h['cost'])} prior evals", flush=True)
    try:
        opt.optimize(x0)
