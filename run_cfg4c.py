...
os.environ.pop("GPUMEEP_KOTTKE", None)   # use area-fraction path (new analytic mirror)
os.environ.pop("MEEP_NO_SUBPIXEL", None) # MEEP eps_averaging ON (default)
...
print("gpumeep |Ey| max=%.5f" % np.abs(gg).max())
