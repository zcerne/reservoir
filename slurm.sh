...
# micromamba pmp env (meep + mpi4py) — replaces the removed miniconda3.
...
$MPIRUN -np $N $PY class_simulation.py --path "$PATH_ARG" --lc-only \
...
$MPIRUN -np $N $PY class_simulation.py --path "$PATH_ARG" --empty-only \
...
if [ -n "$BUILD_T" ]; then
    ...
    $MPIRUN -np $N $PY class_simulation_T.py --path "$PATH_ARG" --build-T \
...
if [ -n "$BUILD_T_LC" ]; then
    ...
    $PY class_cavity_T.py --cavity-path "$PATH_ARG" --setup-lc-only \
        >> "$T_LC_DIR/build_T.log" 2>&1
    echo "=== T_LC build: $PATH_ARG/lc_only ($N processes) ===" | tee -a "$T_LC_DIR/build_T.log"
    $MPIRUN -np $N $PY class_simulation_T.py --path "$PATH_ARG/lc_only" --build-T \
        >> "$T_LC_DIR/build_T.log" 2>&1
fi
